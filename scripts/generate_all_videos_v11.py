"""
Gerar 3 vídeos (PT, EN, ES) v11 usando video_only_EN_fixed.mp4 como base visual.
v11: Vídeo base corrigido com textos visuais:
  - "Accumulated trash" → "Accumulated rubbish"
  - "Cleanup effort" → "Clean-up effort"
Narrações: EN usa v10 (cenas 4,6) + v9 (demais); PT e ES usam v9.
"""
import subprocess
import os
from pydub import AudioSegment

VIDEO_BASE = "/home/ubuntu/video_only_EN_fixed.mp4"
...
# Narrações por idioma
def get_narr_path(lang, scene):
    if lang == "en" and scene in [4, 6]:
        return f"/home/ubuntu/narration_en_v10/cena{scene}.mp3"
    elif lang == "en":
        return f"/home/ubuntu/narration_en_v9/cena{scene}.mp3"
    elif lang == "pt":
        return f"/home/ubuntu/narration_pt_v9/cena{scene}.mp3"
    elif lang == "es":
        return f"/home/ubuntu/narration_es_v9/cena{scene}.mp3"
...
def generate_video(lang):
    print(f"\n{'='*50}")
    print(f"  Gerando vídeo {lang.upper()} (v11 - vídeo base corrigido)")
    print(f"{'='*50}")
    
    srt_file = f"/home/ubuntu/legendas_{lang.upper()}_final_v11.srt"
    output = f"/home/ubuntu/dengue_final_{lang.upper()}.mp4"
    temp_audio = f"/home/ubuntu/temp_audio_{lang}_v11.wav"
    temp_video = f"/home/ubuntu/temp_video_{lang}_v11.mp4"
    
    print("1. Carregando narrações...")
    narrations = {}
    for i in range(1, 8):
        path = get_narr_path(lang, i)
...
print(f"\n{'='*50}")
print("  TODOS OS 3 VÍDEOS v11 GERADOS COM SUCESSO!")
print(f"{'='*50}")
